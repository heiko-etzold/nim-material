...
Wenn die App gestartet wird, zeigt sich ein Bildschirm mit diversen Einstellungsmöglichkeiten für das Nim-Spiel. 
...
So ist es zum Beispiel auch möglich, im Archiv (siehe Abschnitt \@ref(archivieren-und-untersuchen)) einzelne Spiele zu den Notizen zu ziehen und diese dann dort als Bilder zur Verfügung zu haben.

```{r SplitscreenScreenshot, echo=FALSE, fig.cap="Splitscreen mit Verschieben eines Spiels, um es als Bild zur Verfügung zu haben", fig.align='center', out.width='75%'}
knitr::include_graphics("pictures/SplitscreenScreenshot.png")
```
...
* Über **Drag-and-Drop-Gesten** können die **Archiveinträge umsortiert** oder (wenn sie aus der App herausgezogen werden) **als Bilder exportiert** werden. Bitte beachten Sie, dass es bei unterschiedlichen Feldlängen innerhalb des Archivs ggf. zu Verschiebungen in der Darstellung kommen kann. 
...
* **Tippt** man im Archiv ein einzelnes Spiel an, erhält man die Möglichkeit, dieses als Bild **in die Zwischenablage zu kopieren**, es **genauer zu untersuchen** oder es nach einer Bestätigungsabfrage zu **löschen**. Untersucht man es genauer, wird das gespielte Spiel sowie ein weiteres Spiel mit zunächst denselben Einstellungen dargestellt. Die Einstellungen des zweiten Spiels sind nun variierbar, d. h. es kann parallel und vergleichend zum ursprünglich dargestellten Spiel gespielt werden.
...
Die App unterstützt dies dahingehend, dass die Spiele immer entsprechend der zu betrachtenden Parameter (»Wer fing an?«, »Wie viele Plättchen durfte man legen?«, …) umsortiert werden können. 
